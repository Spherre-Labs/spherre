...
pub mod MemberRemoveTransaction {
    ...
    > of IMemberRemoveTransaction<ComponentState<TContractState>> {
        fn propose_remove_member_transaction(
            ...
            // Emit event
            self.emit(MemberRemovalProposed { transaction_id, member: member_address });
    ...
    > of PrivateTrait<TContractState> {
        ...
        fn assert_can_remove_member(
            ...
            let is_voter = permission_control_comp.has_permission(member, Permissions::VOTER);
            let voters_count = account_data_comp.get_number_of_voters();
            // check if the member is not the last voter
            assert(voters_count > 1 || !is_voter, Errors::ERR_CANNOT_REMOVE_LAST_VOTER);
            // Validate that the member is not the last proposer
            let is_proposer = permission_control_comp.has_permission(member, Permissions::PROPOSER);
            let proposers_count = account_data_comp.get_number_of_proposers();
            // check if the member is not the last proposer
            assert(proposers_count > 1 || !is_proposer, Errors::ERR_CANNOT_REMOVE_LAST_PROPOSER);
            // Validate that the member is not the last executor
            let is_executor = permission_control_comp.has_permission(member, Permissions::EXECUTOR);
            let executors_count = account_data_comp.get_number_of_executors();
            // check if the member is not the last executor
            assert(executors_count > 1 || !is_executor, Errors::ERR_CANNOT_REMOVE_LAST_EXECUTOR);
            // validate that the threshold is not equal to current number of voters and the member
            // is not a voter
